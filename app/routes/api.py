...
def _scratch_set_company_hiring_signal(company_id: int, override: HiringSignalOverride, db: Session = Depends(get_db)):
    """TEMPORARY -- one-off backfill for batch 40 (today's Apify discovery-source validation
    test), whose real hiring signal is already known from the job posting title but never
    written to the DB, since /companies/import doesn't take these fields. Delete after use."""
    company = (
        db.query(Company)
        .join(Batch)
