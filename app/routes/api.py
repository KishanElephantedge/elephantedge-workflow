...
LLM_DM_VERIFY_PROMPT = """You are verifying whether a LinkedIn profile genuinely belongs to a \
real, current decision-maker (founder/CEO/president/senior exec) of a SPECIFIC target company.

Target company: "{company_name}" (website: {domain})

Candidate found via a Google search for this company's founder/CEO:
Name: {first_name} {last_name}
Claimed title: {title}

Their LinkedIn profile text:
"{profile_text}"

Does this LinkedIn profile genuinely belong to that person at that specific company? Allow for \
reasonable variations in how the company name appears (abbreviations, "by X" suffixes, product \
name vs. legal/parent name, .com vs .io, etc.) -- but reject if the profile is clearly about an \
unrelated company that just happens to share a similar or identical name. This is a real, \
confirmed failure mode: a search for a company called "Nebi" once matched someone at an \
unrelated, much larger company called "Nebius" purely because the names looked similar.

Return JSON exactly: {{"is_match": true/false, "confidence": "high"/"medium"/"low", "reasoning": "..."}}
Only answer true if you are genuinely confident this is the right person at the right company."""


@router.get("/_scratch/llm-verify-dm/{company_id}")
def _scratch_llm_verify_dm(company_id: int, db: Session = Depends(get_db)):
    from app.apify_client import search_google_ai_overview, search_linkedin_people
    from app.llm_client import generate_json
    from app.phases.free_decision_maker import GOOGLE_LEADER_EXTRACTION_PROMPT, _names_match, _slug_matches

    company = db.query(Company).filter(Company.id == company_id).first()
    if not company:
        raise HTTPException(status_code=404, detail="Company not found")

    apify_key = _get_apify_key_for_diag_v2(db)
    query = f"{company.name} {company.domain} founder CEO"
    try:
        content = search_google_ai_overview(apify_key, query)
    except Exception as e:
        return {"company": company.name, "error": f"google search failed: {e}"}
    if not content:
        return {"company": company.name, "google_ai_overview_text": None}
    ...
    try:
        ...
        return {"company": company.name, "google_ai_overview_text": content, "extraction_error": str(e)}

    out = {"company": company.name, "domain": company.domain, "llm_extraction": extracted, "candidates": []}
    for candidate in (extracted.get("candidates") or []):
        ...
        if not first_name or not last_name:
            continue
        entry = {"first_name": first_name, "last_name": last_name, "title": title, "profiles": []}
        try:
            people = search_linkedin_people(apify_key, first_name, last_name, company.name, max_results=3)
        except Exception as e:
            entry["people_search_error"] = str(e)
            out["candidates"].append(entry)
            continue
        for p in people:
            profile_text = f"{p.get('summary') or ''} {p.get('about') or ''}".strip()
            rule_based_pass = (
                (_names_match(first_name, last_name, p.get("name") or "") or _slug_matches(first_name, last_name, p.get("profileUrl") or ""))
                and company.name.strip().lower() in profile_text.lower()
            )
            llm_verdict = None
            if profile_text:
                try:
                    llm_verdict = generate_json(
                        LLM_DM_VERIFY_PROMPT.format(
                        ...
                        db, ELEPHANT_EDGE_TENANT_ID, max_tokens=200,
                    )
                except Exception as e:
                    llm_verdict = {"error": str(e)}
...
def _get_apify_key_for_diag_v2(db: Session) -> str:
