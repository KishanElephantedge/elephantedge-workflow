...
2. Exact, tenant-scoped, case-insensitive match on Company.name against the signal's
   ...
   matches -> falls through to step 3 only if explicitly enabled (see allow_paid_enrichment).
   Two or more matches -> ambiguous, resolution stops there -- no fuzzy scoring, no guessing.
3. Bounded, OPT-IN-ONLY paid enrichment fallback. Reuses two existing capabilities, unmodified,
...
def resolve_company_for_signal(
    ...
    if signal.company_id is not None:
        ...
        if len(matches) == 1:
        ...
        elif not (signal.company_name_raw or "").strip():
            result = {"status": "unresolved", "method": None, "reason": "no_company_name_raw_available_on_signal", "company_id": None}
        elif allow_paid_enrichment:
            ...
            result = {
                "status": "unresolved",
                "method": None,
                "reason": f"no_exact_name_match_for {signal.company_name_raw!r} and paid enrichment not enabled for this call",
                "company_id": None,
            }
